...
def noisy_Pauli_density(word, lmbda):
    ...
    for i in range(len(word)):
        if word[i] in ['X','Y','Z']:
            p_dict[i] = word[i]
    #print(p_dict) 
    pw = qml.pauli.PauliWord(p_dict)
    iw = qml.pauli.PauliWord({}) #i_word
    P = pw.to_mat(range(len(word)))
    I = iw.to_mat(range(len(word)))
    #print(P)
    #print(I)
    ps1 = qml.pauli.PauliSentence({pw: 2**-len(word), iw: 2**-len(word)})    
    #print(ps1)
    #I = np.eye(2**(len(word)))
    density_p = ps1.to_mat(range(len(word)))
    
    #print(density_p)
    qml.QubitDensityMatrix(density_p,range(len(word)))
    for wire in list(range(len(word))):
...
def maxmix_trace_dist(word, lmbda):
    ...
    def circuit(word,lmbda):
        ...
        return qml.density_matrix(wires=list(range(len(word))))
        
    #print(qml.draw(circuit)(word='XXI', lmbda=1))
    density = circuit(word,lmbda)
    #tr = qml.math.trace(density)
    #print(density)
    maxmix = (2.0**(-1*len(word)))*np.eye(2**(len(word)))
    #print(maxmix)
    dist = 0.5*np.trace(abs_dist(density, maxmix))
    #print(dist)
    return dist
...
def bound_verifier(word, lmbda):
    ...
    """
    p_num = word_dist(word)
    # Put your code here #
    dist = maxmix_trace_dist(word, lmbda)
    bound = (1-lmbda)**p_num
    #print(lmbda,p_num)
    #print(dist, bound)
    return (bound -dist)
