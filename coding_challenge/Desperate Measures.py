...
def heisenberg_trotter(couplings, p, time, depth):
    ...
    chains = [ [0,1],[1,2],[2,3],[3,0]]
    # Put your code here #
    for i in range(depth):
        #for chain in chains:
        #    qml.IsingXX(-2*couplings[0]*time/depth,wires=chain)
        #    qml.IsingYY(-2*couplings[1]*time/depth,wires=chain)
        #    qml.IsingZZ(-2*couplings[2]*time/depth,wires=chain)
        #for j in range(4):    
        #    #X
        #    qml.RX(-2*couplings[3]*time/depth,wires=j)   
        
        for chain in chains:
            #Ixx
            qml.CNOT(wires=chain)
            #qml.DepolarizingChannel(p, wires=chain[0])           
            qml.DepolarizingChannel(p, wires=chain[1])      
            qml.RX(-2*couplings[0]*time/depth,wires=chain[0])
            qml.CNOT(wires=chain)
            #qml.DepolarizingChannel(p, wires=chain[0])           
            qml.DepolarizingChannel(p, wires=chain[1])       
        for chain in chains:      
            #Iyy
            qml.RZ(3*np.pi/2,wires=chain[1])
            qml.CNOT(wires=chain)
            #qml.DepolarizingChannel(p, wires=chain[0])           
            qml.DepolarizingChannel(p, wires=chain[1])     
            qml.RZ(np.pi/2,wires=chain[1])
            qml.RY(-2*couplings[1]*time/depth,wires=chain[0])
            qml.RZ(3*np.pi/2,wires=chain[1])
            qml.CNOT(wires=chain)
            #qml.DepolarizingChannel(p, wires=chain[0])           
            qml.DepolarizingChannel(p, wires=chain[1])     
        ...
        for chain in chains:
            ...
            #qml.DepolarizingChannel(p, wires=chain[0])           
            qml.DepolarizingChannel(p, wires=chain[1])     
            qml.RZ(-2*couplings[2]*time/depth,wires=chain[1])
            qml.CNOT(wires=chain)              
            #qml.DepolarizingChannel(p, wires=chain[0])           
            qml.DepolarizingChannel(p, wires=chain[1])     
        for j in range(4):    
