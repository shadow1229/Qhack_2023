...
def W(alpha, beta):
    ...
    # Put your code here #
    arr = np.array( [[np.sqrt(alpha), -1*np.sqrt(beta)],[np.sqrt(beta),np.sqrt(alpha)]]) / (np.sqrt(alpha+beta))
...
def linear_combination(U, V,  alpha, beta):
    ...
    qml.QubitUnitary(np.transpose(V_t), wires=0)
